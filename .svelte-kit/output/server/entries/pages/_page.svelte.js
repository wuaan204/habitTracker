...
const YearGrid = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return ` <div class="overflow-x-auto select-none pb-3" style="will-change: transform, opacity; transform: translateZ(0); "> <svg${add_attribute("width", svgW, 0)}${add_attribute("height", svgH, 0)} viewBox="${"0 0 " + escape(svgW, true) + " " + escape(svgH, true)}" role="grid" aria-label="${escape(year, true) + " habit tracking grid"}" style="display: block; font-family: inherit; overflow: visible;">${each(monthLabels, ({ month, weekIdx }) => {
